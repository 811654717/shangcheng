      ...
      <div class="overflow" v-if="this.paymentTotal != 0">
        <table>
          <tr v-for="(item, index) in paymentData" :key="index">
            ...
            <td style="width:50px">{{item.productCount}}
            </td>
            ...
            <td style="width:130px">{{item.buyerName}}
            </td>
            ...
            <td style="width:150px">{{item.shippingTime}}</td>
            <td style="width:160px">{{item.deliveryNumber}}</td>
            <td style="width:150px">
              <el-button v-if="item.mailWay == 1" type="primary">
                <a style="color:#ffffff" :href="item.deliveryUrl" target="_blank">查看物流</a>
                </el-button>
              <el-button @click="affirm(item.orderNumber)" v-else type="primary">确认收货</el-button>
            </td>
          </tr>
        </table>
      </div>
    ...
    <!-- 弹窗 -->
      <div v-show="popupTrue" class="popup">
        <div class="popupDiv">
          ...
          <el-carousel height="540px" :interval="0" arrow="always" style="width:76%;margin:20px auto;text-align: center;" :autoplay='false' >
  ...
  methods: {
        ...
        Vue.axios
          .get(
            `/yybMallOrders/queryOrdersListByShopUserId?userId=${this.userId}&orderNumber=${
              ...
              this.distributionId
            }&productAddStartTime=${this.k}&productAddEndTime=${this.j}&orderStatus=3`
          )
          .then(res => {
            console.log(res);
    ...
    search(d, b, p, k, j) {
      ...
      this.spinShow = true;
        Vue.axios
          .get(
            `/yybMallOrders/queryOrdersListByShopUserId?userId=${1}&orderNumber=${
              this.orderReference
            }&pageNum=${this.paymentPageNum}&pageSize=${
              this.paymentPageSize
            }&productName=${this.title}&mailWay=${
    ...
    affirm(i){
      console.log(i);
       Vue.axios({
          method: "post",
          url: "/yybMallOrders/updateOrderStatusToSuccessByOrderNumber",
          data: `orderNumber=${i}`,
          headers: {
    ...
    // //查看物流
    // logistics(num,code){
    //   window.location.href = `https://www.kuaidi100.com/chaxun?com=${code}&nu=${num}`
    //   // this.url = `https://www.kuaidi100.com/chaxun?com=${code}&nu=${num}`
    // },
    //点击出现的商品图弹窗
    Carousel(i) {
      this.popupTrue = true;
